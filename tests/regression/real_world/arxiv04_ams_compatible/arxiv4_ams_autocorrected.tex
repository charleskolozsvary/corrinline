% Scanned by scansub v2.74.0 on Thu May 14 19:02:25 EDT 2026
%
% Publication type: journal
%
% Detected hyperams package.
%
% Found 209 \label's, 80 \ref's, 3 \eqref's and 22 \cref's
%
% Bibliography:
%     Found 121 \cite's
%     Found thebibliography environment.
%
% Warnings:
%     Blank line before [ on line 1383
%     Blank line before [ on line 1394
%     Blank line before [ on line 1673
%     Blank line before [ on line 2345
%     Blank line before \end{proposition} on line 2479
%     Blank line before \end{remark} on line 545
%     Definition inside body on line 1230
%     Definition inside body on line 2246
%     Definition inside body on line 2568
%     Definition inside body on line 2647
%     Definition inside body on line 2648
%     Explicit period at end of section title on line 530
%     Explicit period at end of section title on line 666
%     Found 1 occurrence of (\ref{})
%     Found 1 occurrence of .$ (move period outside of math?)
%     Found 1 occurrence of ... (replace by \dots?)
%     Found 18 occurrences of 'arXiv'
%     Found 18 occurrences of 'http' (url?)
%     Found 6 occurrences of 'www' (url?)
%     Invalid email '\href{mailto:dan.petersen@math.su.se}{dan.petersen@math.su.se}' on line 482
%     Invalid email '\href{mailto:rocalucio@imj-prg.fr}{rocalucio@imj-prg.fr}' on line 479
%     Invalid email '\href{mailto:sinan.yalin@univ-angers.fr}{sinan.yalin@univ-angers.fr}' on line 485
%     Label 'dualschurfunctor' in unnumbered math context on line 621
%     Label 'dualschurfunctor-pro' in unnumbered math context on line 625
%     Label 'schurfunctor' in unnumbered math context on line 617
%     Suspicious \text contents '-' on line 1585
%     Suspicious \text contents '-' on line 1637
%     Suspicious \text contents '-' on line 1650
%     Suspicious \text contents '-' on line 1675
%     Suspicious \text contents '-' on line 1679
%     Suspicious \text contents '-' on line 1683
%     Suspicious \text contents '-' on line 1710
%     Suspicious \text contents '-' on line 1727
%     Suspicious \text contents '-' on line 1734
%     Suspicious \text contents '-' on line 1756
%     Suspicious \text contents '-' on line 1768
%     Suspicious \text contents '-' on line 1777
%     Suspicious \text contents '-' on line 1786
%     Suspicious \text contents '-' on line 1797
%     Suspicious \text contents '-' on line 1834
%     Suspicious \text contents '-' on line 1838
%     Suspicious \text contents '-' on line 1851
%     Suspicious \text contents '-' on line 1863
%     Suspicious \text contents '-' on line 2017
%     Suspicious \text contents '-' on line 2025
%     Suspicious \text contents '-' on line 2058
%     Suspicious \text contents '-' on line 2084
%     Suspicious \text contents '-' on line 2098
%     Suspicious \text contents '-' on line 2146
%     Suspicious \text contents '-' on line 2157
%     Suspicious \text contents '-' on line 2165
%     Suspicious \text contents '-' on line 2181
%     Suspicious \text contents '-' on line 2208
%     Suspicious \text contents '-' on line 661
%     Suspicious \text contents '-' on line 727
%     Suspicious \text contents '-' on line 735
%     Suspicious \text contents '-' on line 743
%     Suspicious \text contents '-' on line 777
%
% Suspicious packages:
%     adjustbox (Y)
%     cases (Y)
%     cleveref (Y [load *after* hyperams])
%     enumitem (Y)
%     mathalfa (Y)
%     mathtools (Y)
%     tikz (Y)
...
%     Num eqnarray* environments: 1
%
% Tokens in forbidden context:
%     " in suspicious position (lines 1626 and 2548)
%     \text used in inline math (lines 777, 1710, 1727 and 1756)
%     \textsf in math (use \mathsf instead?) (line 2529)
%     ~ in math mode (lines 514, 516, 560, 562, 564, 579, 581, 590, 650, 661, 727, 743, 779, 785, 793, 811, 822, 841, 850, 880, 889, 899, 908, 1042, 1048, 1052, 1076, 1095, 1113, 1120, 1177, 1183, 1217, 1234, 1265, 1274, 1281, 1289, 1296, 1308, 1331, 1335, 1379, 1384, 1390, 1395, 1426, 1430, 1434, 1438, 1484, 1499, 1509, 1518, 1525, 1533, 1563, 1617, 1650, 1661, 1667, 1715, 1719, 1753, 1768, 1780, 1800, 1830, 1834, 1838, 1851, 1863, 1951, 1952, 1958, 1967, 1973, 1981, 1986, 2002, 2017, 2021, 2025, 2037, 2045, 2058, 2066, 2076, 2080, 2084, 2098, 2146, 2157, 2165, 2208, 2231, 2350, 2394, 2409 and 2469)
%
...
% Tracked commands:
%     Num ar commands: 10
%     Num cref commands: 22
%     Num textcolor commands: 3
%
% No binary characters found.

\documentclass{proc-l}
\usepackage[shortlabels]{enumitem}% (Y)
\usepackage{colonequals}
\usepackage{mathrsfs}
\usepackage{amssymb}
\usepackage{mathrsfs}
\usepackage[bb=boondox]{mathalfa}% (Y)
\usepackage{graphicx}
\usepackage{tikz}% (Y)
\usepackage{tikz-cd}% (Y)
\usepackage{dsfont}
\usepackage{cases}% (Y)
\usepackage{mathtools}% (Y)
\usepackage{adjustbox}% (Y)
\usepackage[all]{xy}

\usepackage{ebgaramond}% (U)

\usepackage{hyperams}
\usepackage[capitalize]{cleveref}% (Y [load *after* hyperams])
...
\address{Victor Roca i Lucio, Universit{\'{e}} Paris Cit{\'{e}} and Sorbonne Universit{\'{e}}, CNRS, IMJ-PRG, F-75013 Paris, France}
...
\address{Sinan Yalin, Universit{\'{e}} d'Angers}
...
Many constructions in higher category theory allow one to build new $\infty$-categories out of existing ones: localization, stabilization, sheaf categories, the Lurie tensor product, etc. When the existing $\infty$-categories are presented by point-set models, it is natural to ask for a point-set model for the result of the construction. Here is an important example. Suppose that $(\mathbf C, \otimes)$ is a {monoidal} $\infty$-category. Let $\mathrm{Alg}(\mathbf C)$ denote the $\infty$-category of $\mathbb E_1$-algebras in $\mathbf C$. If $\mathbf C \simeq \mathsf C[\mathsf W^{-1}]$, can we write down a point-set model of $\mathrm{Alg}(\mathbf C)$ in terms of $\mathsf C$ and $\mathsf W$? Under certain hypotheses on $\mathsf C$ and $\mathsf W$, the answer %%
%% Correction 0, page 2 (AUTOCORRECTED) [ ]
%% Selection: "is yes1<Replace>:</Replace>  Theorem 1.1"
%% Comment:   "."
%%
%% Correction 1, page 2 [ ]
%% Selection: "is ye<Highlight>s1:</Highlight>  Theorem 1.1"
%% Comment:   "Place footnote after sentence period."
%%
%% Correction 2, page 2 [ ]
%% Selection: "yes1:  Theorem <Highlight>1.1 </Highlight>(Lurie). Suppose"
%% Comment:   "Remove \sc font."
%%
%% Correction 8, page 2 (AUTOCORRECTED) [ ]
%% Selection: "for demonstrational <Replace>porpoises</Replace>. 2(another"
%% Comment:   "purposes"
%%
%⭣ ⭣ ⭣ 
is yes\footnote{This footnote is present for demonstrational purposes.}.

\begin{theorem}[{Lurie%%
%⭡ ⭡ ⭡  END of corrections 0, 1, 2, 8
}]% Theorem 1.1.
\label{Theorem:1.1} \label{lurie theorem} Suppose that $(\mathsf C,\mathsf W)$ underlies a combinatorial monoidal model category, which %%
%% Correction 3, page 2 (AUTOCORRECTED) [ ]
%% Selection: "satisfies either<Remove>:</Remove> (a) all"
%% Comment:   ""
%%
%⭣ ⭣ ⭣ 
satisfies either (a) %%
%⭡ ⭡ ⭡  END of correction 3
all objects %%
%% Correction 4, page 2 (AUTOCORRECTED) [ ]
%% Selection: "are cofibrant<Replace>,</Replace> (b) it"
%% Comment:   " or"
%%
%⭣ ⭣ ⭣ 
are cofibrant or (b) %%
%⭡ ⭡ ⭡  END of correction 4
it is left proper, symmetric monoidal, satisfies the monoid axiom, and is cofibrantly generated by cofibrations between cofibrant objects. %%
%% Correction 5, page 2 [ ]
%% Selection: "Alg(C) [W−1]<Caret>  where on t"
%% Comment:   ","
%%
%% Correction 9, page 2 (AUTOCORRECTED) [ ]
%% Selection: "of example<Replace>).</Replace>  "
%% Comment:   ".)"
%%
%⭣ ⭣ ⭣ 
Then\footnote{(another footnote for the sake of example.)}
\begin{equation}
...
This is %%
%% Correction 10, page 2 [ ]
%% Selection: "essentially unique. <Highlight>We refer to [Lur17, Section 4.1.8] for more details.</Highlight> Theorem 1.1"
%% Comment:   "Multi-line annotation."
%%
%⭣ ⭣ ⭣ 
a \emph{rectification} theorem: it says that every $\mathbb E_1$-algebra can be rectified to a strictly associative object on the point-set level, and that this rectification is essentially unique. We refer to \cite[Section 4.1.8]{HigherAlgebra} for more details. \cref{lurie theorem} is not the only rectification theorem in the literature: %%
%⭡ ⭡ ⭡  END of correction 10
see e.g.\ \cite{haugseng-rectification,HinichRectification,PavlovScholbach18} for extensions to algebras over more general operads and to enriched settings.

\begin{remark}% Remark 1.2.
\label{Remark:1.2}\label{strategy remark}
The proofs of all these rectification results are somewhat similar in flavor. One uses the Barr--Beck--Lurie theorem to show that both sides of \eqref{eq: lurie equivalence} are monadic over $\mathbf C$. Then one proves an isomorphism between the two monads: in both cases, the monad is given by the usual %%
%% Correction 7, page 2 [ ]
%% Selection: "→ a  n≥0 X⊗n<Caret>  "
%% Comment:   "."
%%
%⭣ ⭣ ⭣ 
formula
\begin{equation}
\label{classical free algebra} X \mapsto \coprod_{n \geq 0} X^{\otimes n}
\end{equation}% Equation (2)
(On %%
%⭡ ⭡ ⭡  END of correction 7
the point-set level this formula is only valid if one restricts $X$ to the full subcategory of cofibrant objects.)
\end{remark}

\subsection{Rectification of coalgebras.}% Section 1.2.
\label{Section:1.2} This paper is not about \emph{algebras}, but about \emph{coalgebras}.
Let us consider again a monoidal $\infty$-category $(\mathbf C,\otimes)$.

\begin{definition}% Definition 1.3.
\label{Definition:1.3}\label{def:coalgebra}The $\infty$-category of \emph{$\mathbb E_1$-coalgebras} in $\mathbf{C}$ is defined as $\mathrm{Coalg}(\mathbf C) \coloneqq \mathrm{Alg}(\mathbf C^{\mathrm{op}})^{\mathrm{op}}$.
\end{definition}
...
\subsection{Main results}% Section 1.3.
\label{Section:1.3} The goal of this paper is to prove such rectification results for coalgebras over cofibrant operads, in the differential graded setting, over a field. In the associative case, our main theorem says the following.

\begin{theoremintro}% Theorem A.
\label{Theorem:A}\label{thm: main theorem associative case}
...
\begin{remark}% Remark 1.8.
\label{Remark:1.8}The definition of an $\mathcal A_\infty$-coalgebra is less standardized in the literature than the definition of an $\mathcal A_\infty$-algebra. To be clear, for us an $\mathcal A_\infty$-coalgebra is a graded vector space $C$ and a degree $-1$ derivation of the \emph{completed} tensor algebra $\smash{\widehat T}(C[-1])$, which squares to zero. This amounts to a collection of maps
\[
...
\begin{definition}% Definition 1.9.
\label{Definition:1.9}\label{definition of P-coalg}
...
\begin{theoremintro}% Theorem B.
\label{Theorem:B}Let $\mathscr{O}$ be a (classical) $\infty$-operad. There is an equivalence of $\infty$-categories
\[
...
\begin{theoremintro}% Theorem C.
\label{Theorem:C}\label{thm: introrectification}
...
\subsection{Some ideas of the proof of the main theorem.}% Section 1.4.
\label{Section:1.4}
...
\begin{remark}% Remark 1.10.
\label{Remark:1.10}It is certainly natural to expect that a version of \cref{thm: introrectification} is true also if $\kk$ is an arbitrary ring. Furthermore, there should be a version outside the differential graded setting (e.g.~under hypotheses like those of \cref{lurie theorem}). This is the subject of ongoing work.
\end{remark}

\subsection{Applications and perspectives}% Section 1.5.
\label{Section:1.5}
The following result is a celebrated theorem of Mandell, shown in \cite{Mandell01}:

\begin{theorem}[{Mandell}]% Theorem 1.11.
\label{Theorem:1.11}\label{mandell}
...
\begin{expectation}% Expectation 1.12.
\label{Expectation:1.12}\label{expectation2}
...
\begin{theorem}[{Bachmann--Burklund}]% Theorem 1.13.
\label{Theorem:1.13}\label{bb} Fix a prime $p$. Consider the singular chain functor $X \mapsto C_*(X;\overline{\mathbb{F}}_p)$ from the $\infty$-category of spaces, to the $\infty$-category $\mathrm{CAlg}(\mathbf D(\overline{\mathbb{F}}_p)^{\mathrm{op}})^{\mathrm{op}}$. It is fully faithful when restricted to  nilpotent connected $p$-complete spaces.
\end{theorem}
...
\begin{remark}% Remark 1.14.
\label{Remark:1.14}If one tries to prove Expectation \ref{expectation2} directly, at the point-set level, by dualizing Mandell's arguments, then one runs into issues like whether the category of $\mathbb E_\infty$-coalgebras localized at quasi-isomorphisms is comonadic over $\mathbf D(\overline{\mathbb{F}}_p)$ --- a property which is automatic for $\mathrm{CAlg}(\mathbf D(\overline{\mathbb{F}}_p)^{\mathrm{op}})^{\mathrm{op}}$. Thus it seems in a sense that a main obstruction to proving a coalgebraic version of Mandell's theorem was a lack of good point-set models, an obstruction which Burklund--Bachmann circumvented by carrying out the entire argument at the $\infty$-categorical level.
\end{remark}
...
\begin{theoremintro}% Theorem D.
\label{Theorem:D}\label{thm: presenting chains with the coalgebra structure}
...
\begin{corollary}% Corollary 1.15.
\label{Corollary:1.15}Let $\kk$ be a separably closed field of characteristic $p > 0$. Let $X$ be a pointed nilpotent simplicial set.
\begin{enumerate}[label=(\arabic*),start=1]
\item The derived unit of the adjunction
\[
...
\subsection*{Acknowledgements} We wish to warmly thank Gr{\'{e}}gory Ginot for several discussions that led to this paper, and in particular for suggesting the proof of Proposition \ref{prop: isofibration}. The first author also wishes to thank Geoffroy Horel, Brice Le Grignou and Damien Lejay for discussions about these and related topics.
...
\section{Point-set coalgebras}% Section 2.
\label{Section:2}
...
\subsection{Differential graded operads, algebras, and coalgebras}% Section 2.1.
\label{Section:2.1}\label{sec:1-operads}
...
\begin{definition}[{dg operad}]% Definition 2.1.
...
\begin{notation}% Notation 2.2.
\label{Notation:2.2}Let $V$ and $W$ be two chain complexes. We denote by $[V,W]$ the graded module of graded maps between $V$ and $W$, together with the differential given by $\partial(f)= d_W \circ f -(-1)^{|f|}f \circ d_V$. The construction $[-,-]$ defines a canonical self-enrichment of the category of  chain complexes.
\end{notation}

\begin{example}[{Endomorphism operad}]% Example 2.3.
\label{Example:2.3}
...
\begin{example}[{Coendomorphism operad}]% Example 2.4.
\label{Example:2.4}
...
\begin{definition}[{dg $\oneop$-algebra}]% Definition 2.5.
...
\begin{definition}[{dg $\oneop$-coalgebra}]% Definition 2.6.
...
\begin{example}[{Cocommutative coalgebras}]% Example 2.7.
\label{Example:2.7}
...
\subsection{Defining coalgebras via the dual Schur functor}% Section 2.2.
\label{Section:2.2}\label{set:1-dual schur}
...
\label{Lemma:2.8}\label{definition: P-coalgebra}
...
\label{Remark:2.9}In the terminology of \cite{anelcofree2014}, $\widehat{\mathrm{S}}^c(\oneop)$ is a lax comonad, and dg $\oneop$-coalgebras are equivalent to coalgebras over this lax comonad.
\end{remark}

\subsection{Comonadicity of coalgebras over an operad}% Section 2.3.
\label{Section:2.3}
...
\begin{theorem}[{\cite[Theorem 2.7.11]{anelcofree2014}}]% Theorem 2.10.
\label{Theorem:2.10}\label{thm: cofree coalgebra over an operad}
...
\subsection{Coadmissible operads}% Section 2.4.
\label{Section:2.4}
...
\begin{definition}[{Coadmissible operad}]% Definition 2.11.
...
\subsection{Cofibrant operads}% Section 2.5.
\label{Section:2.5}
...
\begin{proposition}% Proposition 2.15.
\label{Proposition:2.15}\label{prop: cofibrant operads are coadmissible}\leavevmode
\begin{enumerate}
...
\begin{corollary}% Corollary 2.17.
\label{Corollary:2.17}\label{corollary: devissage along cells of 1-categories of P-coalgebras}\leavevmode
\begin{enumerate}
...
\section{$\infty$-categorical coalgebras over operads in an enriched setting}% Section 3.
\label{Section:3}
...
\subsection{Symmetric sequences and operads}% Section 3.1.
\label{Section:3.1}
...
\begin{definition}[{$\infty$-operad}]% Definition 3.1.
...
\begin{remark}[{Point-set models for enriched in chain complexes $\infty$-operads}]% Remark 3.2.
\label{Remark:3.2}
...
\begin{remark}[{About the rectification of $\mathcal{P}$-algebras in chain complexes}]% Remark 3.3.
\label{Remark:3.3}
...
\subsection{Defining coalgebras over an (enriched) $\infty$-operad}% Section 3.2.
\label{Section:3.2}To a symmetric sequence $M$, we can associate a \emph{dual Schur functor} $\widehat{\mathbf{S}}^c(M)$ in $\mathbf{End}(\mathbf{D})$,  an $\infty$-categor\-ic\-al analogue of the point-set {dual Schur functor} from \cref{set:1-dual schur}. This construction  defines a functor
\[
...
\label{Lemma:3.4}There is an adjunction
\[
\begin{tikzcd}[column sep=5pc,row sep=3pc]
...
\label{Lemma:3.6}For any object $V$ in $\mathbf D$, the functor $\mathrm{p}\left([V,-]\right)$ is right adjoint to $c(V) ~ \widehat{\otimes} ~ -$.
\end{lemma}
\begin{proof}
...
\label{Lemma:3.7}Let $M$ be a symmetric sequence. There is a natural weak equivalence of functors
\[
...
\label{Lemma:3.8}\label{lemma: enriched pro-adjunction}
...
\label{Lemma:3.9}\label{lemma: pro-tensor and pro-enrichment}
...
\begin{proposition}% Proposition 3.10.
\label{Proposition:3.10}The functor
\[
\lim_k \mathrm{p}\widehat{\mathbf{S}}^c_{\leq k}(-): \mathbf{sSeq}(\mathbf{D})^{\mathsf{op}} \longrightarrow \mathbf{End}(\mathbf{pro}(\mathbf{D}))
\]
...
\end{align*}
where the third equivalence follows from Lemma \ref{lemma: pro-tensor and pro-enrichment} and the forth from Lemma \ref{lemma: enriched pro-adjunction}. It follows directly from the above that
\[
...
\begin{definition}[{$\mathscr{P}$-coalgebra}]% Definition 3.11.
\label{Definition:3.11}\label{def: infinity categorical P-coalgebras}
...
\begin{remark}% Remark 3.12.
\label{Remark:3.12}Given an object $V$ in $\mathbf{D}$, a $\mathscr{P}$-coalgebra structure on $V$ should be equivalent to a map of operads from $\mathscr{P}$ to a \textit{coendomorphism operad} of $V$. However, as far as we know, no such construction exists yet in the $\infty$-categorical setting with the desired properties.
\end{remark}

\subsection{D{\'{e}}vissage of the $\infty$-category of coalgebras over an operad}% Section 3.3.
\label{Section:3.3}
...
\label{Lemma:3.13}\label{lemma: pro-Schur functor preserves limits}
The functor
\[
...
\label{Proposition:3.14}\label{prop: sends free operad to cofree monad}
The functor
\[
\widehat{\mathbf{S}}^c_{\mathrm{pro}}(-): \mathbf{sSeq}(\mathbf{D})^{\mathsf{op}} \longrightarrow \mathbf{End}(\mathbf{pro}(\mathbf{D}))
\]
...
\begin{corollary}% Corollary 3.15.
\label{Corollary:3.15}\label{corollary: coalgebras over a free infinity operad}
...
\begin{proposition}% Proposition 3.16.
\label{Proposition:3.16}\label{prop: taking infinity categories of coalgebras preserves limits}
The functor
\[
...
\begin{corollary}% Corollary 3.17.
\label{Corollary:3.17}Let $\mathscr{P}$ be an $\infty$-operad. The $\infty$-category of $\mathscr{P}$-coalgebras is comonadic over the base $\infty$-category $\mathbf{D}$.
\end{corollary}
...
\subsection{Comparison with the non-enriched case}% Section 3.4.
\label{Section:3.4}
...
\begin{definition}[{$\mathscr{O}$-coalgebras}]% Definition 3.18.
\label{Definition:3.18}\label{Def: O-coalgebras}
...
\begin{remark}% Remark 3.19.
\label{Remark:3.19}Recall that if $\mathbf{C}$ is a symmetric monoidal $\infty$-category, so is $\mathbf{C}^{\mathsf{op}}$. Hence it makes sense, using Lurie's definition of algebras over an $\infty$-operad, to consider $\mathscr{O}$-algebras in $\mathbf{C}^{\mathsf{op}}$.
\end{remark}
...
\label{Theorem:3.20}\label{thm: comparison with coalgebras in the sense of Lurie}
...
\]
On the other hand, the $\infty$-category of $F(\mathscr{O})$-coalgebras in $\mathbf{pro}(\mathbf{D})$ is given by coalgebras over the following comonad:
\begin{align*}
...
\section{Presenting homotopy coalgebras by point-set models}% Section 4.
\label{Section:4}

\vspace{2.5pc}

A major issue in dealing with coalgebras is that the construction of the cofree coalgebra functor is extremely complicated. To bypass this problem, we use the d{\'{e}}vissages of the categories of coalgebras that exist both at the 1-categorical and at the $\infty$-categorical level. This allows us to cut the problem into pieces: we first construct point-set models for coalgebras over free operads, and then we glue them along cell attachments to give point-set models for any quasi-free operad. This allows us to give point-set models for $\infty$-categorical coalgebras in terms of coalgebras in terms of coalgebras over any cofibrant operad.

\subsection{Point-set models for coalgebras over an endofunctor}% Section 4.1.
\label{Section:4.1}\label{Subsection: point-set models for coalgebras over endofunctors}
...
\subsubsection{Coalgebras over an endofunctor}% Section 4.1.1.
\label{Section:4.1.1} We start by considering an endofunctor
\[
F: \mathsf{Ch}(\kk) \longrightarrow \mathsf{Ch}(\kk)
\]
on the underlying category of chain complexes.

\begin{definition}[{$F$-coalgebras}]% Definition 4.1.
\label{Definition:4.1}\label{definition: coalgebras over an endofunctor}
...
\label{Lemma:4.2}\label{lemma: 1-categorical coalgebras over an endofunctor as a pullback}
...
\begin{remark}% Remark 4.3.
\label{Remark:4.3}If the functor $F$ preserves quasi-isomorphisms, then the pullback of Lemma \ref{lemma: 1-categorical coalgebras over an endofunctor as a pullback} is a pullback in relative categories as well. See Appendix \ref{Appendix A: Relative categories} for more on relative categories.
\end{remark}

\subsubsection{Coalgebras over endofunctors in the $\infty$-categorical setting}% Section 4.1.2.
\label{Section:4.1.2} We consider now an endofunctor at the $\infty$-categorical level
\[
...
\begin{definition}[{$\mathrm{T}$-coalgebras}]% Definition 4.4.
\label{Definition:4.4}\label{definition: infinity categorical coalgebras over an endofunctor}
...
\begin{definition}[{$F_\xi$-coalgebras}]% Definition 4.5.
...
\label{Lemma:4.6}\label{lemma: xi-coalgebras}
...
\label{Remark:4.7}Here, a homotopy equivalence of relative categories is the data of two functors going in opposite directions whose composites are homotopic to the identity. The notion of homotopy used is the one determined by $(-) \times\mathsf{1}^{\simeq}$, considered in \cite{BarwickKan1}.
\end{remark}
...
\label{Corollary:4.8}\label{cor:piquasicateq}
...
\label{Theorem:4.9}\label{thm: rectification of coalgebras over an endofunctor}
Let
\[
F: \mathsf{Ch}(\kk) \longrightarrow \mathsf{Ch}(\kk)
\]
...
\subsection{Point-set models for coalgebras over a free operad}% Section 4.2.
\label{Section:4.2}
...
\label{Proposition:4.10}\label{prop: rectification of coalgebras over free operads}
...
\subsection{Gluing $\infty$-categories of coalgebras along cell attachments}% Section 4.3.
\label{Section:4.3}
...
\begin{theorem}% Theorem 4.11.
\label{Theorem:4.11}\label{thm: key theorem}
...
\begin{remark}[{About the proof strategy}]% Remark 4.12.
\label{Remark:4.12}
...
\subsection{Proof of Theorem \ref{thm: key theorem}}% Section 4.4.
\label{Section:4.4}\label{S: the gluing proof}
...
\label{Theorem:4.13}\label{thm: fibrationquasicat}
...
\subsubsection{Isofibration property at the homotopy category level}% Section 4.4.1.
\label{Section:4.4.1} Here we prove that $\ho{\iota^k(p)^*}$ satisfies the first condition of Proposition \ref{prop:fibrationff}.

\begin{proposition}% Proposition 4.14.
\label{Proposition:4.14}\label{prop: isofibration}
Let $p \geq 0$ and $k \in \mathbb{Z}$, the restriction functor
\[
...
\subsubsection{Fullness and faithfulness properties of the restriction $\infty$-functor}% Section 4.4.2.
\label{Section:4.4.2} We show that the functor $\iota^k(p)$ is fully faithful at the $\infty$-categorical level by explicitly identifying this functor.

\begin{proposition}% Proposition 4.15.
\label{Proposition:4.15}Let $p \geq 0$ and $k \in \mathbb{Z}$. The functor $\iota^k(p)^*$ induces a fully faithful $\infty$-functor $i_1^*N_{\xi}\iota^k(p)^*$.
\end{proposition}
...
\subsection{Point-set models for coalgebras over a quasi-free operad}% Section 4.5.
\label{Section:4.5}
...
\label{Proposition:4.16}\label{prop: rectification for cell cofibrant operads}
...
\subsection{Point-set models for coalgebras over a cofibrant operad}% Section 4.6.
\label{Section:4.6}

Finally, we conclude this section by extending the previous results to all cofibrant dg operads.

\begin{theorem}% Theorem 4.17.
\label{Theorem:4.17}\label{thm: rectification for cofibrant operads}
...
\section{Applications: point-set models for non-finite type $p$-adic homotopy types}% Section 5.
\label{Section:5}
...
\subsection{Homotopy types as $\mathbb{E}_\infty$-coalgebras}% Section 5.1.
\label{Section:5.1}
...
\begin{theorem}[{\cite[Theorem 1.2]{bachmann2024}}]% Theorem 5.1.
\label{Theorem:5.1}\label{thm: bachmann-burklund}
...
\label{Proposition:5.3}There is an equivalence of $\infty$-categories
\[
...
\label{Theorem:5.4}\label{thm: point-set model for the chains functor}
Let $\kk$ be a field. The functor
\[
C_*(-;\kk): \mathsf{sSet} \longrightarrow \Omega \mathrm{B} \mathcal{E}\text{-}\mathsf{coalg}~,
\]
...
\label{Corollary:5.5}\label{cor: derived unit is an Fp equivalence}
...
\subsection{Lie-type models}% Section 5.3.
\label{Section:5.3}
...
\begin{theorem}% Theorem 5.6.
\label{Theorem:5.6}Let $X$ be a pointed nilpotent simplicial set. The unit of adjunction
\[
\eta_X: X \qi \mathcal{R}_*\mathcal{L}_*(X)
\]
is an equivalence in homology with coefficients in $\mathbb{F}_p$.
...
\label{Theorem:5.7}Let $X$ be a simplicial set and let $Y$ be a nilpotent simplicial set. There is a weak equivalence of simplicial sets
\[
...
\begin{remark}[{A combinatorial presentation of the homotopy groups of a space}]% Remark 5.8.
\label{Remark:5.8}
...
\section{Appendix. Relative categories and complete Segal spaces}% Section A.
\label{Section:A}\label{Appendix A: Relative categories}
...
\subsection{Relative categories, complete Segal spaces and the subdivided nerve}% Section A.1.
\label{Section:A.1} We recall the definition a relative category and the existence of a model structure on all relative categories, called the Barwick--Kan model structure, which presents the the $\infty$-category of $\infty$-categories. We refer to \cite{BarwickKan1} for more details.

\begin{definition}[{Relative category}]% Definition A.1.
...
\begin{remark}% Remark A.2.
\label{Remark:A.2}Our notations differ from those in \cite{BarwickKan1}. The relative categories that we write as $\mathsf{p}$ and $\mathsf{p}^\simeq$ are written as $\check{\mathsf{p}}$ and $\hat{\mathsf{p}}$, respectively, in \textit{op.cit.}
\end{remark}

\begin{theorem}[{\cite{BarwickKan1}}]% Theorem A.3.
\label{Theorem:A.3}
...
\begin{theorem}[{\cite[Main Theorem]{Meier16}}]% Theorem A.4.
\label{Theorem:A.4}
...
\subsection{Internal homs in relative categories and bisimplicial sets}% Section A.2.
\label{Section:A.2}\label{subsection: internal homs}
...
\begin{definition}% Definition A.5.
\label{Definition:A.5}The internal hom for the cartesian structure on bisimplicial sets is defined as follows. Given two bisimplicial sets $X$ and $Y$, we denote by $Y^X$ the bisimplicial set whose bisimplices are given by
\[
...
\begin{definition}% Definition A.6.
\label{Definition:A.6}The internal hom for the cartesian structure on relative categories is defined as follows. Given two relative categories $\mathsf{C}$ and $\mathsf{D}$, we denote by $\mathsf{D}^\mathsf{C}$ the relative category
\begin{itemize}[label=-]
...
\subsection{The lax simplicial model structure on relative categories}% Section A.3.
\label{Section:A.3}
...
\begin{proposition}% Proposition A.7.
\label{Proposition:A.7}The model category of bisimplicial sets $\mathsf{bisSets}$ forms a simplicial model category in which, given two bisimplicial sets $X$ and $Y$, the simplicial mapping space is defined by the $0$th row of the internal hom. That is,
\[
...
\begin{lemma}% Lemma A.8.
\label{Lemma:A.8}\label{lemma:laxmonL}
...
\begin{remark}% Remark A.9.
\label{Remark:A.9}The lax monoidality can also be seen from the construction of the subdivision functor: on a standard simplex, it is defined as the nerve of the category of non-degenerate simplices. The definition is then extended to any simplicial set $X$ by setting
\[
...
\begin{theorem}% Theorem A.10.
\label{Theorem:A.10}\label{thm: lax simplicial tensoring and cotensoring}
The model category $\mathsf{RelCat}$ forms a lax simplicial model category endowed with:

\begin{enumerate}[label=(\arabic*),start=1]
\item the tensoring defined by $\mathsf{C} \times L(X)$ for any relative category $\mathsf{C}$ and simplicial set $X$, which preserves colimits in each variable and defines a left Quillen bifunctor;

\item cotensoring defined by $\mathsf{C}^{L(X)}$ for any relative category $\mathsf{C}$ and simplicial set $X$;

\item simplicial hom spaces defined by
\[
...
\end{itemize}
\end{proof}
\begin{remark}% Remark A.11.
\label{Remark:A.11}Note that $L(\Delta^n)=\xi\mathsf{n}^{\simeq}$, so the theorem above provides functorial simplicial and cosimplicial resolutions in the model category of relative categories $\mathsf{RelCat}$. They are given, for any relative category $\mathsf{C}$, by $\mathsf{C} \times \xi (\bullet)^{\simeq}$ and $\mathsf{C}^{\xi(\bullet)^{\simeq}}$.
\end{remark}

\subsection{Cylinders, path objects and arrow categories in relative categories and complete Segal spaces}% Section A.4.
\label{Section:A.4}
...
\subsubsection{The usual path and cylinder objects}% Section A.4.1.
\label{Section:A.4.1} Let us first recall the usual definition of a cylinder or a path object in a general model category.

\begin{definition}% Definition A.12.
\label{Definition:A.12}Let $\mathsf{C}$ be a model category.
\begin{enumerate}[label=(\arabic*),start=1]
\item A good cylinder object of $X \in \mathrm{ob}(\mathsf{C})$ is a factorization of the codiagonal map $\mathrm{id} \amalg \mathrm{id}: X \amalg X \longleftarrow X$ as
\[
...
\item A good path object $Y\in \mathrm{ob}(\mathsf{C})$ is a factorization of the diagonal map $(\mathrm{id},\mathrm{id}): Y \longrightarrow Y \times Y$ as
\[
...
\begin{proposition}% Proposition A.13.
\label{Proposition:A.13}\label{prop: goodpath}
...
\begin{corollary}% Corollary A.14.
\label{Corollary:A.14}\label{cor:evaluationfibration}\leavevmode

\begin{enumerate}[label=(\arabic*),start=1]
\item The endofunctor $(-)^{\Delta[0,1]}$ of bisimplicial sets defines a functorial good path object for any fibrant object in the Rezk model structure.

\item  The endofunctor $(-)^{\xi\mathsf{1}^{\simeq}}$ of relative categories defines a functorial good path object for any fibrant object in the Barwick--Kan model structure.
\end{enumerate}

In particular, we get that:

\begin{enumerate}[label=(\arabic*),start=1]
\item For any fibrant relative category $\mathsf{C}$, the map  $(\mathrm{ev}_0,\mathrm{ev}_1):\mathsf{C}^{\xi\mathsf{1}^{\simeq}}\rightarrow \mathsf{C}\times \mathsf{C}$ is a fibration in $\mathsf{RelCat}$;

\item For any complete Segal space $X$, the map  $(\mathrm{ev}_0,\mathrm{ev}_1):X^{\Delta[0,1]}\rightarrow X\times X$ is a fibration in $\mathsf{bisSet}.$
\end{enumerate}
\end{corollary}

\begin{remark}% Remark A.15.
\label{Remark:A.15}\label{rem: quasicatpath}
...
\subsubsection{Models for the $\infty$-category of morphisms}% Section A.4.2.
\label{Section:A.4.2}
...
\begin{remark}% Remark A.16.
\label{Remark:A.16}Notice that the difference between $\mathrm{Fun}^{iso}(\Delta^1,\mathsf{C})$  and $\mathrm{Fun}(\Delta^1,\mathsf{C})$ in Remark \ref{rem: quasicatpath} (where $\mathsf{C}$ is a quasi-category) parallels the difference between functors $\mathsf{1}^{\simeq}\rightarrow\mathsf{C}$ and $\mathsf{1}\rightarrow\mathsf{C}$ (where $\mathsf{C}$ is a relative category), and also parallels the difference between $\mathrm{Hom}_{\mathsf{bisSet}}(\Delta[0,1],\mathsf{C})$ and $\mathrm{Hom}_{\mathsf{bisSet}}(\Delta[1,0],\mathsf{C})$ (where $\mathsf{C}$ is a complete Segal space). In all of these situations, the first object models equivalences in $\mathsf{C}$ and the second one general arrows in $\mathsf{C}$.
\end{remark}
...
\begin{proposition}% Proposition A.17.
\label{Proposition:A.17}\label{prop: evaluationfibrations}\leavevmode
\begin{enumerate}[label=(\arabic*),start=1]
\item For any fibrant relative category $\mathsf{C}$, the map  $(\mathrm{ev}_0,\mathrm{ev}_1):\mathsf{C}^{\xi\mathsf{1}}\rightarrow \mathsf{C}\times \mathsf{C}$ is a fibration in $\mathsf{RelCat}$.

\item For any complete Segal space $X$, the map  $(\mathrm{ev}_0,\mathrm{ev}_1):X^{\Delta[1,0]}\rightarrow X\times X$ is a fibration in $\mathsf{bisSet}$.
\end{enumerate}
\end{proposition}
...
\section{Appendix. Fibrations of quasi-categories}% Section B.
\label{Section:B}\label{Appendix B: fibrations of quasi-categories}
...
\begin{notation}% Notation B.1.
\label{Notation:B.1}If $F$ is a functor  between quasi-categories, we denote by $\ho{F}$ the corresponding functor between homotopy categories, which we implictly view again as quasi-categories via the nerve functor.
\end{notation}

\begin{lemma}% Lemma B.2.
\label{Lemma:B.2}\label{pullbackff}
...
\begin{lemma}% Lemma B.3.
\label{Lemma:B.3}\label{lemma:nerveofhF}
...
\begin{proposition}% Proposition B.4.
\label{Proposition:B.4}\label{prop:fibrationff}
