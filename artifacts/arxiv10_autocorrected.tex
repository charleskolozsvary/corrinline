...
\section{Introduction} We are interested in solving discrete ill-posed inverse problems of the %%
%% Correction 6 (auto) [✓]
%% Annotated text: "+ ϵ with<Caret> A(ytrue)xtrue ="
%% Comment: "\quad" 
%% 
...
%% Correction 3 [ ]
%% Annotated text: "∗Departamento de Matem´atica, Facultad de Ciencias Exactas y Naturales, Universidad de Buenos United Luis A.</Highlight> <Highlight>∗D Aires, †Sc States ‡C Santal Argentina. †School of Mathematical and Statistical Sciences, Arizona State University, Tempe, AZ, Buenos United Luis A.</Highlight> <Highlight>∗De Aires, †Sc States ‡CO Santal (malena.espanol@asu.edu). ‡CONICET–Universidad de Buenos Aires, Instituto de Investigaciones Matem´aticas “Luis nos ed A.</Highlight> <Highlight>∗Depa Aires, Ar †Scho States (m ‡CON Santal´o” (IMAS), Argentina (jeronimo@dm.uba.ar)."
...
%% Correction 7 (auto) [✓]
%% Annotated text: "2. Variable <Replace>Projection Method</Replace>. The basic"
...
%% Correction 8 (auto) [✓]
%% Annotated text: "2.1. Motivating <Replace>E</Replace>xample. We now"
%% Comment: "e" 
%% 
...
We now present an example to motivate the need for %%
%% END of correction 8
the regularization term $\mathcal{R}(\by)$ in \eqref{eq: nlls_regy}, even if $\by$ is of small size. We will show that, without the regularization term, the solution of the minimization problem is trivial (i.e., $\mathbf{y}=\boldsymbol{0}$, and therefore $\mathbf{A}(\mathbf{y})$ becomes the identity matrix, and the solution $\bx=\bb$). 
...
\begin{figure}[htbp]
    ...
    \caption{%%
%% Correction 9 [ ]
%% Annotated text: "Figure 1. <Highlight>(Left) True signals: an edgy sig</Highlight>nal (top) and"
%% Comment: "set text in this caption upright" 
%% 
%% START of correction 9
(Left) True signals: an edgy signal (top) %%
%% END of correction 9
and a smooth signal (bottom), and their corresponding blurred and noisy signals; (Middle) reduced functional $\varphi(\sigma)$ for selected values of $\lambda$; and (Right) convergence of $\sigma_k$ using the built-in MATLAB function \texttt{fminsearch}.}
...
%% Correction 10 (auto) [✓]
%% Annotated text: "U(Σ2 + λ2I)−1ΣU⊤b<Remove>.</Remove>"
%% Comment: "" 
%% 
